...
-(void)createSubViews{
    ...
    self.jianBtn.frame = CGRectMake(0,0, 24, 24);
    [self.jianBtn addTarget:self action:@selector(deleteBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    self.jianBtn.tag = 11;
   // self.jianBtn.backgroundColor = [UIColor redColor];
    [self.jianBtn setImage:[UIImage imageNamed:@"done_reduce_default"] forState:UIControlStateNormal];
    ...
    //numberBg.backgroundColor = [UIColor redColor];
    ...
    self.addBtn.frame = CGRectMake(numberBg.right-5,self.jianBtn.top, 24, 24);
    self.addBtn.tag = 12;
    [self.addBtn setImage:[UIImage imageNamed:@"done_add_default"] forState:UIControlStateNormal];
