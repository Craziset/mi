//
//  songOderViewController1.m
//  magicCamera
//
//  Created by jianpan on 2017/11/14.
//  Copyright © 2017年 张展展. All rights reserved.
//  全部订单
#import "songOderViewController1.h"
...
@interface songOderViewController1 ()<UITableViewDelegate,UITableViewDataSource,PayDelegate>{
    int ToalPage;
    int page;
}
...
@implementation songOderViewController1
-(NSMutableArray *)SuperArrSong{
    if (_SuperArrSong ==nil) {
        self.SuperArrSong = [NSMutableArray array];
    }
...
-(void)Requst:(int)tag{
    ...
    [afnObject POST:API_OrderAll parameters:dicPar success:^(id responseObject) {
...
-(SongFootView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{  //SongFootView
    ...
    if ([[self.SuperArrSong[section] state_str] isEqualToString:@"已取消订单"])
    {
        [foot.StusBtn2 setTitle:@"已取消" forState:UIControlStateNormal];
        foot.StusBtn1.hidden = YES;
        foot.StusBtn2.userInteractionEnabled = NO;
    }else
    {
        [foot.StusBtn2 setTitle:@"取消" forState:UIControlStateNormal];
        foot.StusBtn1.hidden = NO;
        foot.StusBtn2.userInteractionEnabled = YES;
    }
    return foot;
}
...
-(void)requstFootEvent:(NSString *)str withOrderNo:(NSDictionary *)dic{
    [ZJProgressHUD showProgress];
    NSLog(@"%@",[NSString stringWithFormat:@"%@%@",SongMainUrl,str]);
