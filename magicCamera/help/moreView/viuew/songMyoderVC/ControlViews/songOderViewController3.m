...
#import "songOderViewController3.h"
...
@interface songOderViewController3 ()<UITableViewDelegate,UITableViewDataSource>{
    int ToalPage;
    int page;
}
...
@implementation songOderViewController3
...
-(void)Requst:(int)tag{
    ...
    [afnObject POST:API_OrderUnsend parameters:dicPar success:^(id responseObject) {
...
-(void)requstFootEvent:(NSString *)str withOrderNo:(NSDictionary *)dic{
    ...
    [afnObject POST:[NSString stringWithFormat:@"%@%@",SongMainUrl,str] parameters:dic success:^(id responseObject) {
        ...
        [XHToast showBottomWithText:responseObject[@"message"] bottomOffset:150 duration:3.5f];
    } failure:^(id error) {
    } Cahche:NO];
}
@end
