...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        lb = [[UILabel alloc] initWithFrame:CGRectMake(10, 20, 100, 30)];
        lb.text = @"购买数量";
        ...
        bt_add.frame = CGRectMake(self.frame.size.width-10-30, 20,30, 30);
        ...
        bt_add.layer.cornerRadius = 15;
        bt_add.clipsToBounds = YES;
        [self addSubview:bt_add];
        
        tf_count = [[UITextField alloc] initWithFrame:CGRectMake(bt_add.frame.origin.x -45, 20, 40, 30)];
        tf_count.text = @"1";
        tf_count.textAlignment = NSTextAlignmentCenter;
        tf_count.font = [UIFont systemFontOfSize:15];
        tf_count.backgroundColor = [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1];
        [self addSubview:tf_count];
        
        bt_reduce= [UIButton buttonWithType:UIButtonTypeCustom];
        bt_reduce.frame = CGRectMake(tf_count.frame.origin.x -35, 20, 30, 30);
        ...
        bt_reduce.layer.cornerRadius = 15;
        bt_reduce.clipsToBounds = YES;
